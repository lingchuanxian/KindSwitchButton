package lingcx.kind.com.cn.library;
 ...
 * @version 1.0
 * @description
 * @date 2018/5/26 下午5:13
 * @copyright: 2018 www.kind.com.cn Inc. All rights reserved.
 */
public class KindSwitchButton extends View {
    ...
    public KindSwitchButton(Context context) {
        this(context, null);
    }

    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    public KindSwitchButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.KindSwitchButton);
        colorPrimary = a.getColor(R.styleable.KindSwitchButton_primaryColor, DEFAULT_COLOR_PRIMARY);
        colorPrimaryDark = a.getColor(R.styleable.KindSwitchButton_primaryColorDark, DEFAULT_COLOR_PRIMARY_DARK);
        colorOff = a.getColor(R.styleable.KindSwitchButton_offColor, 0xFFE3E3E3);
        colorOffDark = a.getColor(R.styleable.KindSwitchButton_offColorDark, 0xFFBFBFBF);
        colorShadow = a.getColor(R.styleable.KindSwitchButton_shadowColor, 0xFF333333);
        ratioAspect = a.getFloat(R.styleable.KindSwitchButton_ratioAspect, 0.68f);
        hasShadow = a.getBoolean(R.styleable.KindSwitchButton_hasShadow, true);
        isOpen = a.getBoolean(R.styleable.KindSwitchButton_isOpen, false);
        state = isOpen ? STATE_SWITCH_ON : STATE_SWITCH_OFF;
        lastState = state;
        a.recycle();
    ...
    public void setOpen(boolean isOpen) {
        int wishState = isOpen ? STATE_SWITCH_ON : STATE_SWITCH_OFF;
        if (wishState == state) {
            return;
        }
     ...
     * @param isOpen
     */
    public void toggleSwitch(boolean isOpen) {
        if(listener != null){
            if(isOpen){
                listener.toggleToOn();
            }else{
    ...
    private void refreshState(int newState) {
        if (!isOpen && newState == STATE_SWITCH_ON) {
            isOpen = true;
        } else if (isOpen && newState == STATE_SWITCH_OFF) {
            isOpen = false;
        }
        lastState = state;
        state = newState;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (isCanVisibleDrawing) {
            ...
            bRectF.left = bLeft;
            bRectF.right = bRight;
            bRectF.top = sTop + bStrokeWidth / 2;
            bRectF.bottom = sBottom - bStrokeWidth / 2;
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        this.isOpen = ss.isOpen;
        this.state = this.isOpen ? STATE_SWITCH_ON : STATE_SWITCH_OFF;
        invalidate();
    }
